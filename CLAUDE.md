...
- **Support Gemini CLI : la conversion est GÉNÉRÉE, jamais commitée.** Le repo ne contient QUE la version Claude. `scripts/to-gemini.mjs` la traduit et l'installe dans `~/.gemini` chez le fondateur (à l'install, puis à chaque conversation via le hook). Table de conversion : `scripts/gemini-map.mjs`. **Ne jamais créer de fichier `*-gemini.md` en double** — la sync hebdo réécrirait la copie Claude et les deux dériveraient en silence. Si tu ajoutes un agent/skill qui cite un outil Claude (`Read`, `Bash`, `WebSearch`…) ou un événement de hook, **ajoute la correspondance dans `gemini-map.mjs`** : la CI (`to-gemini.mjs --check`) échoue si la conversion laisse passer du Claude.
