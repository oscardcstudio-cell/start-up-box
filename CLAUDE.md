...
- **Marketplace = legacy, non recommandé** : `.claude-plugin/marketplace.json` existe encore mais le chemin `/plugin install` donne un install INCOMPLET (agents + skills seulement, ni harness ni auto-update). Ne pas le promouvoir dans les docs user. Le seul chemin béni est le prompt-installeur de `INSTALL.md`. (Si tu y touches encore : après tout ajout/suppression dans `marketplace.json`, vérifier la cohérence du `plugin.json`.)
