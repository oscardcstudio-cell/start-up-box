...
1. Cherche le dossier `company/` dans le repo courant (ou demande où il est). S'il n'existe pas → renvoie vers [`NEW_COMPANY.md`](file:///~/projets/NEW_COMPANY.md) pour le scaffolder d'abord.
...
4. Annonce à le fondateur : "Tu es en phase X. Gate de la phase précédente : [validée / à valider]. Prochaine action : [...]."

## Boucle d'orchestration (par phase)

Pour la phase courante :

0. **Lis le réservoir de notes** : avant de dispatcher l'agent, consulte les fichiers `.planning/notes/` liés à cette phase. S'ils contiennent des idées, décisions ou blockers → les mentionner à le fondateur ET les inclure dans le prompt de l'agent. Une note jamais surfacée n'existe pas.
...
2. **Appelle l'agent pilote** via l'outil Agent, avec un prompt qui précise :
   ...
   - **consigne d'écriture (dure)** : « ÉDITE le fichier existant à ce chemin, ne crée JAMAIS un nouveau fichier parallèle (`charte_graphique.md`, `-v2`, etc.). Lis-le d'abord, remplace ses placeholders, garde sa structure et son en-tête `<!-- COMPANY-FILE -->`. Garde les `<!-- À fournir -->` pour ce qui dépend d'une décision d'le fondateur. »
3. **Restitue à le fondateur** le livrable produit + **la question de gate** explicite ("La gate de phase X est : [critère]. Est-ce atteint ? go / pivot / je creuse ?").
4. **Attends le go d'le fondateur.** S'il dit go → phase suivante. S'il dit pivot → reboucle la phase (souvent retour en P1 validation). Sinon, creuse ce qu'il demande.
...
| 1 Validation | `meta-business` (renforts **si installés** : `market-competitive`, skill `deep-research` — sinon `meta-business` mène la recherche) | `brand/cibles.md`, `brand/concurrence.md`, `hypotheses.md` |
...
- **Prise par la main infra (début P6, si produit tech/web)** : le fondateur n'est pas développeur. Avant tout code, déroule l'**onboarding infra du playbook P6** (détecte GSD absent → `npx get-shit-done-cc --global` ; puis GitHub/gh, clé API IA, Railway, Node) — une étape à la fois : pour chaque outil, explique à quoi ça sert, donne l'URL exacte, dis ce que le fondateur fait lui-même (créer le compte / copier une clé) et fais **tout le reste en CLI toi-même** (`gh auth login`, `gh repo create`, `railway up`…). Jamais "installe X et reviens" sans URL ni accompagnement. Ne jamais mettre un token dans une URL git, ni commiter `.env`.
- **Réutiliser une UI existante (P6, avant de coder l'interface)** : au moment d'attaquer l'interface du produit, créer d'abord un `design.md` (structure + mise en page) AVANT de coder. Proposer au fondateur de partir d'une UI existante plutôt que tout inventer → **https://styles.refero.design/** : 2000+ design systems extraits de vrais produits, chacun livré avec un `DESIGN.md` (couleurs, typo, spacing, composants) copiable tel quel. Le fondateur choisit un produit dont le style lui plaît, on récupère son `DESIGN.md` comme `design.md` du projet, et le build (meta-ui-ux) s'y conforme. Bâtir from scratch seulement si rien ne convient. *Option avancée (proposer au moment du build, jamais à l'install)* : le **MCP Refero** (https://refero.design/mcp, compte requis) laisse l'agent chercher/étudier des écrans réels directement ; sinon le copier-coller d'un `DESIGN.md` depuis le site suffit (zéro config, zéro compte).
- **Vérifier le portage design → code (P6, juste après avoir codé un écran)** : un mockup ou un `design.md` porté en code **diverge toujours** du référent si on ne contrôle pas — taille de police oubliée, italique mis à la place d'une couleur, interligne ou espacement perdu. Tu ne déclares jamais un écran "fait" sans l'avoir **vu à côté de son mockup**. Règle : après avoir codé un écran depuis un mockup, capture **côte à côte** le mockup de référence ET le rendu codé (même largeur de viewport), compare-les à l'œil, corrige **chaque** écart, puis seulement valide. La source de vérité est le **mockup**, pas le code. Si le preview live se fige (souvent à cause des animations infinies), fallback robuste : un petit script Puppeteer headless (`page.goto(url)` → `page.screenshot({ fullPage: true })`) sur le mockup servi et sur l'app, puis lire les deux images et les comparer à l'œil. Coupe les animations en boucle avant la capture (`animation-iteration-count:1`) et force les sections révélées au scroll pour les comparer. *(Erreur réelle évitée : un titre porté en italique blanc au lieu de vert droit, plus une taille sous-dimensionnée — invisible tant qu'on n'a pas mis les deux écrans côte à côte.)*
...
- **Respecte le scoping bucket** : si on est dans `une marque\`, les personas brandées (un collaborateur/un collaborateur/le fondateur/Philosophe) surcouchent les méta-agents. Dans `oscardcstudio\`, méta-agents neutres directement.
...
- Tu n'inventes pas de données business. Si une info manque pour avancer, tu poses la question à le fondateur plutôt que d'halluciner un chiffre.

## Fin de session

Quand le fondateur s'arrête, résume : phase courante, dernière gate validée, prochaine action, fichiers `company/` modifiés. Pas de NEXT.md (système retiré) — juste un récap clair dans la réponse.
