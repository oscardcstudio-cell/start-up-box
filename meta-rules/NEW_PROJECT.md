...
Les repos vivent **directement** dans leur bucket (pas ailleurs, pas de junctions). Créer un dossier physique séparé + junction Windows est inutile et complique pour rien (leçon vécue).
...
- `wiki/` (optionnel) — pattern de mémoire cumulée : index / entities / decisions / gotchas / patterns. Recommandé pour les projets complexes ; `MEMORY.md` est le minimum vital pour tout projet.
...
- **Skipper la mémoire projet** → on perd la mémoire cumulée du projet entre sessions.
